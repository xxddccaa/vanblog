...
test('docker compose defines the split runtime services', () => {
  for (const service of ['caddy:', 'server:', 'website:', 'admin:', 'waline:', 'mongo:']) {
  ...
  assert.match(compose, /docker\/waline\.Dockerfile/);
});
...
test('mongo stays private to the compose network', () => {
  const mongoSection = compose.match(/\n  mongo:\n([\s\S]*?)(?:\n  [a-z][a-z0-9_-]*:|\n$)/i)?.[1] ?? '';
  assert.ok(mongoSection.length > 0, 'expected to find the mongo service block');
  assert.doesNotMatch(mongoSection, /\n\s+ports:\s*\n/);
  assert.match(mongoSection, /healthcheck:/);
});
