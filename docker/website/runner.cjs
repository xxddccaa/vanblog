...
const startChild = () => {
  child = spawn('node', ['packages/website/server.js'], {
    cwd: '/app/website',
    env: {
      ...process.env,
      ...runtimeEnv,
  ...
  child.on('exit', (code, signal) => {
    ...
    if (!wasManagedRestart) {
      console.warn(`[website-runner] website exited with code=${code} signal=${signal}`);
      child = null;
    }
  });
};
...
server.listen(controlPort, '0.0.0.0', () => {
  console.log(`[website-runner] control server listening on ${controlPort}`);
});

restartChild().catch((error) => {
  console.error('[website-runner] failed to start website', error);
  process.exit(1);
});
