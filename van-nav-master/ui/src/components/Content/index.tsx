...
const Content = (props: any) => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setData(r);
      // Always start with "全部工具" selected
      setCurrTag("全部工具");
    } catch (e) {
      console.log(e);
    } finally {
  ...
  // Filter categories to only show those with tools and always include "全部工具"
  const validCategories = useMemo(() => {
    if (!data.tools || !data.catelogs) {
      return ["全部工具"];
    }
    
    // Get a set of categories that have at least one tool
    const categoriesWithTools = new Set<string>();
    categoriesWithTools.add("全部工具"); // Always include this one
    
    // Add categories that have tools
    data.tools.forEach((tool: any) => {
      if (tool.catelog) {
        categoriesWithTools.add(tool.catelog);
      }
    });
    
    // Filter the catelogs to only include those with tools
    return data.catelogs.filter((cat: string) => categoriesWithTools.has(cat));
  }, [data]);
