...
try {
  ...
  if (!dashboardPath) {
    throw new Error('未找到 Waline dashboard.js 文件');
  }
  
  console.log('找到 dashboard.js 文件:', dashboardPath);
} catch (error) {
  console.error('查找 Waline dashboard.js 文件失败:', error.message);
  process.exit(1);
}
...
try {
  ...
  if (modified) {
    ...
    throw new Error('未找到可修复的 Waline dashboard serverURL 配置');
  }
