...
function downloadMermaidSvg(container: HTMLElement, themeMode: MermaidThemeMode) {
...
async function downloadMermaidPng(container: HTMLElement, themeMode: MermaidThemeMode) {
  const svg = getMermaidSvg(container);
  if (!svg) {
    throw new Error('svg not found');
  }
...
function handleExportError(format: 'png' | 'svg', error: unknown) {
  ...
  message.error(format === 'png' ? 'PNG 导出失败，请优先导出 SVG' : 'SVG 导出失败');
}
...
function enhanceMermaidExportControls(markdownBody: HTMLElement, themeMode: MermaidThemeMode) {
