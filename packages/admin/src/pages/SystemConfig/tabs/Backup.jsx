...
export default function (props) {
  ...
  const importDataTypes = [
    {
      name: '✅ 内容数据',
      desc: '文章、草稿、动态：增量导入，ID冲突时自动重新分配，相同标题则更新',
    },
    ...
    { name: '✅ 脑图数据', desc: '按 _id / 标题恢复脑图数据，适合完整迁移' },
    { name: '✅ 访问统计', desc: '统计数据：覆盖导入历史访问记录' },
    {
      name: '⚠️ 用户信息与站点信息',
      desc: '当目标站点为空时自动完整恢复；若目标站点已有内容，则继续保护当前登录账号和站点配置',
    },
  ];
      ...
      <Alert
        ...
        description={
          <div>
            ...
            <p>
              <strong>安全导入：</strong>
              当目标站点为空时会执行完整恢复；如果目标站点已经有内容，则仍会保护当前登录账号和站点配置。
            </p>
            <p>
              <strong>图片文件：</strong>
            ...
            <p>
              <strong>增量导入：</strong>
              文章等内容数据采用智能增量导入，ID冲突时自动重新分配，避免数据覆盖。
            </p>
          </div>
        }
          ...
          description={
            <div>
              <p>
                ...
                将永久删除博客中的所有内容，包括文章、草稿、动态、分类、导航等，此操作不可逆转！
              </p>
      ...
      <div style={{ marginTop: 24, padding: 16, background: '#f6f8fa', borderRadius: 6 }}>
        ...
        <ul style={{ margin: 0, paddingLeft: 20 }}>
          ...
          <li>
            ...
            导入到全新站点时会恢复用户和站点配置；导入到已有内容的站点时会自动保护当前登录账号和站点信息
          </li>
          <li>
            <Text strong>智能合并：</Text>定制化代码采用增量导入，不会清空现有的CSS/JS代码
          </li>
          <li>
