...
import { fetchInit, fetchInitStatus } from '@/services/van-blog/api';
...
import { hasStoredAdminToken } from '@/utils/adminSession';
...
const InitPage = () => {
  ...
  const [checkingInitStatus, setCheckingInitStatus] = useState(true);

  useEffect(() => {
    let alive = true;

    const syncInitStatus = async () => {
      try {
        ...
        if (res?.statusCode === 200 && res?.data?.initialized) {
          history.replace(hasStoredAdminToken() ? '/' : '/user/login');
          return;
        }
      } finally {
        if (alive) {
