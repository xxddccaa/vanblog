...
- 作者
  ...
  默认为登录者本人，下拉框内的选择范围包括 _所有协作者的昵称_ 以及后台站点设置中的 _作者名称_。
...
- 自定义路径名
  ...
  如果将自定义路径名设置成 `如何部署VanBlog`，发布后即可通过 `/post/如何部署VanBlog` 访问。

  :::

- 置顶优先级：数字，留空或者为 0 表示不置顶

- 标签

  每篇文章可以有多个标签，用于索引文章。

  标签不需要提前创建，你可以在需要标签的表单里看到所有标签或者直接创建新标签：

  ![设置标签](https://pic.mereith.com/img/f96db83327831a83b5eb7b010be0f431.clipboard-2022-08-15.png)

- 分类

  选择分类前需要先创建分类，具体请参考 [分类管理](./tag.md#分类管理)

- 是否加密 & 密码

  选中后，前台的文章将被隐藏，需要输入密码之后才能看到内容。密码会在后端验证，只有匹配时才会把文章数据返回前端。
...
- 是否隐藏
  ...
  选择隐藏后，前台不会显示该文章，也不会把它计入公开统计，但后台仍可正常查看与取消隐藏。

## 导入文章

你可以点击 `导入` 按钮导入 Markdown 文件来创建文章，暂不支持其他类型文件。
...
或者也可以在文章页面的表格点击编辑按钮，跳转到编辑器后，再点击右上角的 `修改信息` 按钮进行修改。

## 导出文章

你可以在文章管理页面操作栏下拉菜单中点击 `导出` 来导出文章，或者通过编辑器中的导出按钮来导出文章。

导出时会自动映射信息到 Markdown 文件的 FrontMatter 中。

现在也支持批量导出，多选后点击批量导出即可。

## 删除文章

你可以选择删除某篇文章。当前删除本质上属于软删除，但后台没有提供误删恢复入口。

如果确实需要恢复，请优先从备份恢复，或自行检查数据库中的数据。
