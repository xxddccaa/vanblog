...
VanBlog 默认集成了 [Waline 评论系统](https://waline.js.org/)。在当前官方 compose 拓扑中，Waline 会作为独立容器启动，但前台展示、后台管理、配置同步仍然由 VanBlog 统一接入，开箱即用。
...
在当前实现里，Waline 会以独立容器运行，后台页面继续通过 `iframe` 内嵌 Waline 管理页面。

![评论管理](https://pic.mereith.com/img/dd7792a91f5a3b945ee2b261b06f666a.clipboard-2022-08-25.png)

配置信息会由后端的服务读取并同步到独立 Waline 容器中；本地开发场景下仍保留了内置启动 fallback。
