...
与 [文章管理](./article.md) 类似，你可以在草稿管理页面中对草稿进行管理。
...
一般而言，更推荐从草稿开始写作，确认内容无误后再发布到文章列表中。
...
- 标签
  ...
  标签不需要提前创建，你可以在需要标签的表单里看到所有标签或者创建新的标签：
...
你可以点击 `导入` 按钮导入 Markdown 文件来创建草稿，暂不支持其他类型文件。
...
可以点击草稿表格操作栏下拉菜单中的修改信息进行修改，或者在编辑页面点击右上角操作中的 `修改信息` 按钮进行修改。

## 发布草稿

当草稿已经完善到可以发布的程度，可以在编辑页面点击右上角操作菜单中的 `发布草稿` 进行发布。具体选项含义可以参考 [文章管理](./article.md)。

![发布草稿](https://pic.mereith.com/img/4aadf867768d04fb906085532664c0df.clipboard-2023-02-27.png)

草稿发布后会从草稿管理页面消失，并出现在文章管理页面。

## 导出草稿

你可以在草稿管理页面操作栏下拉菜单中点击 `导出` 来导出草稿，或者通过编辑器中的导出按钮来导出草稿。
...
你可以选择删除某篇草稿。当前删除本质上属于软删除，但后台没有提供误删恢复入口。

如果确实需要恢复，请优先从备份恢复，或自行检查数据库中的数据。

## 批量操作

草稿管理页面支持批量操作，可以批量删除或者批量导出。
