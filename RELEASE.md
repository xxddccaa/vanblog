...
当前代码基线已经推进到 `v1.5.6`，默认镜像仓库继续固定为长期保留的 `kevinchina/deeplearning`。
...
3. 确认根目录 `package.json` 的版本号正确，例如 `1.5.6`，并统一成发布标签 `vX.Y.Z`。
...
```bash
kevinchina/deeplearning:vanblog-caddy-v1.5.6-<image-id>
kevinchina/deeplearning:vanblog-server-v1.5.6-<image-id>
kevinchina/deeplearning:vanblog-website-v1.5.6-<image-id>
kevinchina/deeplearning:vanblog-admin-v1.5.6-<image-id>
kevinchina/deeplearning:vanblog-waline-v1.5.6-<image-id>
```
...
- 自动读取 `package.json` 版本，例如 `1.5.6`，并规范成 `v1.5.6`
...
版本文档、Wiki、GitHub Release 中给用户展示的 compose 示例，默认仍写版本别名 tag，例如 `kevinchina/deeplearning:vanblog-caddy-v1.5.6`。
...
bash scripts/release-publish.sh --version v1.5.6 --skip-tests
```
...
```bash
pnpm release:latest --version v1.5.6 --image-id <image-id>
```

或：

```bash
bash scripts/release-publish.sh sync-latest --version v1.5.6 --image-id <image-id>
```
...
2. 把根目录 `package.json` 版本改成目标版本，例如 `1.5.6`。
...
gh release create v1.5.6 \
  --title "v1.5.6" \
  --notes-file docs/releases/v1.5.6.md
```
...
export VANBLOG_RELEASE_SUFFIX=v1.5.6-<image-id>
...
- `vanblog-caddy-v1.5.6-<image-id>`
- `vanblog-server-v1.5.6-<image-id>`
- `vanblog-website-v1.5.6-<image-id>`
- `vanblog-admin-v1.5.6-<image-id>`
- `vanblog-waline-v1.5.6-<image-id>`
...
```bash
export VANBLOG_RELEASE_SUFFIX=v1.5.6
```
